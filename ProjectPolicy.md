...
- WEB検索やAI検索に出てきてほしいので、ルートの *.html は公開する。
- サブフォルダは公開しない。CSS/JSでの処理は公開しない。
...
- 設定項目の説明ボタン  設定項目ラベルの左に小さなボタンを置く。web_asset アイコンを使用する。
    ここをクリックやタップすると、<dialog>で項目を説明する文章（Tips）が表示される。
    <dialog>で項目を説明する文章（Tips）は、文頭に"・"を置いた短めの文書の箇条書きで説明する。
    このTipsボタンの見た目は、枠線と背景色と文字色はラベルと同じ。Tipsボタンの右側の枠線は、文字色と同じ。
...
## ドキュメント（*.md ファイル）のファイル名規則

- 全て大文字の `*.md`（例: `README.md`, `CLAUDE.md`, `AGENTS.md`）は、外部の慣習に従うファイルに使用する。
- プロジェクト内部の `*.md`（例: `ProjectPolicy.md`, `ScoreFormat.md`）は、大文字小文字混在のファイル名を使用する。
