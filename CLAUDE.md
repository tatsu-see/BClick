...
- releases\v*.* フォルダ内にあるファイルは過去版なので、releases\v*.*\index.html をトップ画面とし、ここで過去版を使用可能にしておく。
- www と releases\next と releases\v*.* フォルダは、それぞれ独立した版として、基本的には影響しあわないようにする。
...
- iPhone のキャッシュ対策に "js / css" が更新されたら、更新された "js, css" を参照しているパスの後ろに、"?v=更新日" を入れる。
  ...
  例）AIに依頼するときのプロンプト（v*.*は適宜編集）
  releases/next フォルダのソースが最新です。releases/v*.* のソースが旧です。
  これを比較して更新のあったreleases/next フォルダの *.js , *.css のソースにおいて、これら *.js , *.css を参照する箇所に
   ?=今日の日付 としてほしいです。（iphoneのキャッシュ対策）
